...
class SDWS_Primary_Nav_Walker extends Walker_Nav_Menu {
	...
	public function start_el( &$output, $data_object, $depth = 0, $args = null, $id = 0 ) {
		...
		if ( $has_children && 0 === $depth ) {
			$btn_style = 'padding:0.5rem 0.875rem; font-size:0.9rem; font-weight:400;'
			...
			if ( 0 === $depth ) {
				$a_style = 'padding:0.5rem 0.875rem; font-size:0.9rem; font-weight:400;'
					. ' letter-spacing:0.02em; color:#000; text-decoration:none; display:block;';
				if ( $is_current ) {
					$a_style .= ' border-bottom:2px solid #000;';
				}
			} else {
				$a_style = 'padding:0.75rem 1rem; font-size:0.9rem; color:#000; text-decoration:none; display:block;';
			}
